...
import com.alltypeservsforservdefs_9_0_0_1.classicmodels.service.Table13Service;
...
public Table13 createTable13(@RequestPart("wm_data_json") Table13 table13, @RequestPart(value = "column2", required = false) MultipartFile _column2) {
    ...
    @ApiOperation(value = "Retrieves content for the given BLOB field in Table13 instance" )
    @RequestMapping(value = "/{id}/content/{fieldName}", method = RequestMethod.GET, produces="application/octet-stream")
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public DownloadResponse getTable13BLOBContent(@PathVariable("id") Integer id, @PathVariable("fieldName") String fieldName, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, @RequestParam(value="download", defaultValue = "false") boolean download) {

        LOGGER.debug("Retrieves content for the given BLOB field {} in Table13 instance" , fieldName);

        if(!WMRuntimeUtils.isLob(Table13.class, fieldName)) {
            throw new TypeMismatchException("Given field " + fieldName + " is not a valid BLOB type");
        }
        Table13 table13 = table13Service.getById(id);

        return WMMultipartUtils.buildDownloadResponseForBlob(table13, fieldName, httpServletRequest, download);
    }
    ...
    @ApiOperation(value = "Updates the Table13 instance associated with the given id.This API should be used when Table13 instance fields that require multipart data.") 
    @RequestMapping(value = "/{id:.+}", method = RequestMethod.POST, consumes = {"multipart/form-data"})
    @WMAccessVisibility(value = AccessSpecifier.APP_ONLY)
    public Table13 editTable13(@PathVariable("id") Integer id, MultipartHttpServletRequest multipartHttpServletRequest) throws EntityNotFoundException {
        Table13 newTable13 = WMMultipartUtils.toObject(multipartHttpServletRequest, Table13.class, "classicmodels");
        newTable13.setId(id);

        Table13 oldTable13 = table13Service.getById(id);
        WMMultipartUtils.updateLobsContent(oldTable13, newTable13);
        LOGGER.debug("Updating Table13 with information: {}" , newTable13);

        return table13Service.update(newTable13);
    }
